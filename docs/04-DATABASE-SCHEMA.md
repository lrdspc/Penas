# 🗄️ Schema Supabase - SQL Completo

Este documento contém o schema completo do PostgreSQL com todas as tabelas, índices e RLS policies.

## 📋 Tabelas

O banco de dados consiste em 9 tabelas principais organizadas em três categorias:

### 1. Usuários e Relacionamentos
- `users` - Perfis de usuários (trainers e alunos)
- `trainer_students` - Relação entre trainers e alunos

### 2. Conteúdo e Treinos
- `exercises` - Biblioteca de exercícios
- `workouts` - Templates de treinos
- `workout_exercises` - Exercícios dentro dos treinos

### 3. Execução e Progresso
- `student_workouts` - Treinos atribuídos a alunos
- `workout_sessions` - Sessões de treino executadas
- `session_exercises` - Séries por exercício

### 4. Avaliações e Sync
- `assessments` - Avaliações físicas
- `push_subscriptions` - Subscrições de push notifications
...
-- Trainers podem ver seus alunos
...
- `user_type` define se é trainer ou aluno
- `profile_photo_url` armazena URL do Supabase Storage
- `last_login` usado para tracking de atividade
...
- `invite_token` é usado para onboarding de alunos (6 dígitos)
- `status` pode ser: pending, active, inactive
- UNIQUE constraint evita relações duplicadas
- CASCADE DELETE remove relações quando usuário é deletado
...
- `muscle_groups` é um array PostgreSQL para buscas multi-categoria
- `video_storage_path` aponta para arquivo no Supabase Storage
- `difficulty` pode ser: beginner, intermediate, advanced
- GIN index para busca eficiente em arrays
...
- `workout_type` define o tipo de treino
- `is_template` indica se pode ser reusado
- `last_used` para ordenar treinos mais recentes primeiro
...
- `order_num` define a ordem de execução
- `sets` e `reps` com constraints para valores válidos
- `rest_seconds` em segundos (15-300 = 15s a 5min)
...
- `status` rastreia o estado do treino para o aluno
- `scheduled_time` permite agendar treinos para horário específico
- `completed_date` null enquanto não completado
...
- `offline_mode` indica se foi criado offline
- `sync_status` rastreia status de sincronização
- `device_info` armazena informações sobre o dispositivo
...
- `reps_per_set` é um array para armazenar reps de cada série
- `weight_kg` é o peso sugerido, `weight_used` é o peso realmente usado
- `sets_completed` pode ser 0 se não completou nenhuma série
...
- Todas as medidas são em centímetros (cm)
- `body_fat_percent` e `muscle_mass_percent` são percentuais
- `photos_urls` é um array de URLs do Supabase Storage
- Cálculos (IMC, gordura em kg, etc.) são feitos na aplicação
...
- `subscription` armazena o objeto completo da PushSubscription API
- `platform` para segmentação de notificações
- UNIQUE constraint evita subscrições duplicadas

---

## 📊 Tabela: offline_sync_queue (Background Sync)
...
- `action` define o tipo de operação a sincronizar
- `retry_count` para tentativas automáticas (máx. 3)
- `last_error` armazena mensagem de erro em falhas
- Itens com status 'failed' devem ser revisados manualmente
...
Todas as tabelas têm RLS ativado para garantir:
- ✅ Alunos só veem seus próprios dados
- ✅ Trainers só veem dados de seus alunos
- ✅ Acesso granular por operação (SELECT, INSERT, UPDATE, DELETE)
- ✅ Validação em nível de banco (não apenas na aplicação)

### CASCADE DELETE

- `trainer_students`: CASCADE DELETE remove relações quando usuário é deletado
- `exercises`: CASCADE DELETE remove exercícios quando trainer é deletado
- `workout_exercises`: CASCADE DELETE remove exercícios quando workout é deletado
- `session_exercises`: CASCADE DELETE remove séries quando sessão é deletado
- `student_workouts`: SET NULL quando workout é deletado (mantém histórico)

### Constraints e Checks

- CHECK constraints em colunas para garantir valores válidos
...
## 📈 Cálculos Automáticos (Aplicação)
...
```typescript
const calculateFatKg = (weightKg: number, bodyFatPercent: number): number => {
...
```typescript
const calculateLeanMass = (weightKg: number, fatKg: number): number => {
...
-- Inclui todas as RLS policies acima
```
...
1. **Sempre usar RLS** - Nunca desabilitar RLS
2. **Usar índices** - Criar índices para queries frequentes
3. **Usar prepared statements** - Nunca concatenar SQL
4. **Validar na aplicação** - Double-check de constraints

### Performance

1. **Evitar N+1 queries** - Usar joins quando apropriado
2. **Limitar resultados** - Usar LIMIT/PAGINATION
3. **Otimizar JSONB** - Usar GIN indexes para JSONB
4. **Usar materialized views** - Para agregações complexas

### Segurança

1. **Nunca expor service_role_key** - Apenas no servidor
2. **Validar permissões** - Verificar user_type antes de operações
3. **Log operações sensíveis** - Registrar mudanças importantes
4. **Implementar rate limiting** - Limitar requisições por usuário

---

## 🎓 Conclusão

Este schema PostgreSQL foi cuidadosamente projetado para:
- ✅ **Escalabilidade** - Índices otimizados e relações eficientes
- ✅ **Segurança** - RLS granular em todas as tabelas
- ✅ **Flexibilidade** - Arrays e JSONB para dados complexos
- ✅ **Performance** - Queries otimizadas e caching inteligente
- ✅ **Integridade** - Constraints e checks robustos

O schema está pronto para produção e pode ser facilmente migrado para o Supabase. 🚀
