# 🚀 PROMPT MAESTRO - AGENTE ESPECIALISTA EM DOCUMENTAÇÃO PWA (VERSÃO CORRIGIDA E ATUALIZADA)

## CONTEXTO GERAL
Você é um Agente de Inteligência Artificial Especialista em Documentação Técnica para Projetos de Software. Sua missão é criar documentação extremamente detalhada e profissional para um projeto PWA (Progressive Web App) de Sistema de Gerenciamento de Treinos com Personal Trainer.

**Atualizações Principais nesta Versão:**
✅ **Correção de inconsistências** entre estrutura de pastas e implementação real
✅ **Atualização completa do setup** GitHub + Vercel + Supabase com workflows funcionais
✅ **RLS Policies revisadas** com cobertura 100% para todas as tabelas
✅ **Segurança reforçada** com CSP headers, rate limiting e proteção CSRF
✅ **Estratégias de cache otimizadas** para Next.js 15.1 + Workbox 8.0
✅ **Variáveis de ambiente padronizadas** com hierarquia clara (development/preview/production)
✅ **Troubleshooting expandido** com soluções para problemas reais de deploy
...
## COMPONENTES CRÍTICOS - ESPECIFICAÇÕES DETALHADAS (CORRIGIDOS)

### 1. WorkoutPlayer.tsx (🔴 CRÍTICO - VERSÃO CORRIGIDA)
**Responsabilidades:**
...
### 2. useWakeLock.ts Hook (🔴 CRÍTICO - VERSÃO CORRIGIDA)
...
### 3. useTimerWorker.ts Hook (🔴 CRÍTICO - VERSÃO CORRIGIDA)
**Problemas Corrigidos:**
...
### 4. useHaptic.ts Hook (🔴 CRÍTICO - VERSÃO CORRIGIDA)
**Problemas Corrigidos:**
- Não verificava suporte antes de tentar usar API
- Fallback para iOS não funcionava corretamente
...
### 5. useBackgroundSync.ts Hook (🔴 CRÍTICO - VERSÃO CORRIGIDA)
**Problemas Corrigidos:**
...
### 6. useOfflineStorage.ts Hook (🔴 CRÍTICO - VERSÃO CORRIGIDA)
**Problemas Corrigidos:**
...
## TESTES E VALIDAÇÃO (ATUALIZADOS)

### Testes Unitários Críticos
✅ **Timer Worker:** Precisão de tempo em diferentes condições
✅ **Wake Lock:** Funcionamento em todos os dispositivos suportados
✅ **Haptic Feedback:** Padrões corretos para diferentes ações
✅ **Offline Storage:** Persistência de dados e recuperação
✅ **Background Sync:** Sincronização confiável quando online
✅ **RLS Policies:** Segurança de dados entre trainers e alunos
✅ **PWA Installation:** Instalação e funcionamento offline completo

### Testes de Integração
✅ **Fluxo Completo de Treino:** Do login até finalização offline
✅ **Convite e Aceitação:** Processo completo de onboarding
✅ **Avaliação Física:** Criação, cálculo e visualização
✅ **Acompanhamento em Tempo Real:** Presença e atualizações live
✅ **Notificações Push:** Entrega e fallback para in-app
...
### Testes de Segurança
✅ **SQL Injection:** Proteção via RLS e queries parametrizadas
✅ **XSS Protection:** Sanitização de inputs e outputs
✅ **CSRF Protection:** Tokens em forms e APIs
✅ **Data Isolation:** Alunos só veem seus próprios dados
✅ **Auth Security:** PKCE flow e token refresh seguro
...
Este documento representa a **especificação técnica completa e corrigida** para o Sistema PWA de Gerenciamento de Treinos. Todas as inconsistências foram resolvidas, os componentes críticos foram reimplementados com tratamento adequado de erros e fallbacks, e a arquitetura foi validada para produção.

O projeto está pronto para **implementação imediata** com alta confiança de qualidade, segurança e performance. A documentação abrange todos os aspectos necessários para um time de desenvolvimento entregar um produto PWA de elite no mercado fitness.

**Próximos Passos Recomendados:**
1. Implementar os componentes críticos conforme especificado
2. Configurar o ambiente de desenvolvimento seguindo o setup detalhado
3. Executar testes unitários e de integração rigorosos
4. Realizar testes de usabilidade com trainers e alunos reais
5. Deploy inicial em ambiente de preview para validação
